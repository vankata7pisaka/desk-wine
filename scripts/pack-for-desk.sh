...
# ── каквото Wine отваря чак по време на РАБОТА ──────────────────────────────
#
# Проследяването по-долу върви по `NEEDED` — тоест по това, с което всеки файл
# е СВЪРЗАН. Wine обаче отваря част от библиотеките си с `dlopen`, чак когато
# наистина потрябват, и тогава името им не стои никъде в ELF таблиците. За
# проследяването те просто не съществуват и мълчаливо не влизат.
#
# Измерено на 14.08.2026: инсталаторът на Steam мина докрай, но самият Steam
# остана завинаги на „Checking for available updates…". В дневника пишеше:
#
...
# отвън изглежда просто като бавна.
#
# Затова тези влизат В ОПАШКАТА изрично. Оттам нататък обичайното
# проследяване им намира и техните собствени зависимости.
RUNTIME_LIBS="libgnutls.so"

echo "── библиотеки, които се отварят по време на работа ──"
for name in $RUNTIME_LIBS; do
  found=$(find "$SYSROOT/lib" -maxdepth 1 -name "$name*" | head -1)
  [ -n "$found" ] || { echo "   ! липсва $name — Wine ще остане без TLS" >&2; exit 1; }
  cp -aL "$found" "$PACK/lib/$name"
  seen[$name]=1
  queue+=("$PACK/lib/$name")
  echo "   + $name"
done
...
# Версията носи и ОТПЕЧАТЪК НА СЛОЯ, не само версията на Wine.
#
# По този ред `WineContainer` решава дали да разпъва наново. Докато беше само
# `11.3-<комит>`, всяка промяна в ОПАКОВАНЕТО оставаше невидима: изходният код
# на Wine е същият, значи и редът е същият, значи Desk решава, че вече го има
# и не пипа нищо. Хванато на 14.08.2026 — билдът, който най-после сложи
# libgnutls вътре, щеше да излезе с непроменена версия и телефонът щеше да си
# остане със стария Wine без TLS.
#
# Отпечатъкът е на самите файлове, затова се смята ПРЕДИ архива: xz няма как
# да даде един и същ изход два пъти подред.
LAYER=$(find "$PACK" -type f -exec sha256sum {} + | sort -k2 | sha256sum | cut -c1-7)
echo "${WINE_VERSION:-11.3}-${WINE_SHA:-dev}-$LAYER" > "$PACK/.version"
